...
data class HomeMusicUiState(
    ...
    val radioArtists: List<ArtistID3> = emptyList(),
...
class HomeViewModel(
    ...
    private val _radioArtists = MutableStateFlow<List<ArtistID3>>(emptyList())
    ...
    val musicUiState: StateFlow<HomeMusicUiState> = combine(
        _discoverSongs, _similarTracks, _bestOfArtists, _radioArtists, _topSongs,
        ...
        _shares, _sectorConfig, _isLoading
    ) { args ->
        ...
        HomeMusicUiState(
            ...
            radioArtists = args[3] as List<ArtistID3>,
            topSongs = args[4] as List<Child>,
            starredTracks = args[5] as List<Child>,
            starredAlbums = args[6] as List<AlbumID3>,
            starredArtists = args[7] as List<ArtistID3>,
            newReleases = args[8] as List<AlbumID3>,
            flashbackYears = args[9] as List<Int>,
            mostPlayedAlbums = args[10] as List<AlbumID3>,
            recentlyPlayedAlbums = args[11] as List<AlbumID3>,
            recentlyAddedAlbums = args[12] as List<AlbumID3>,
            pinnedPlaylists = args[13] as List<Playlist>,
            shares = args[14] as List<Share>,
            sectorConfig = args[15] as List<HomeSector>,
        ...
        viewModelScope.launch {
            when (sectorId) {
                Constants.HOME_SECTOR_DISCOVERY -> {
                    songRepository.getRandomSample(10, null, null).observeForever { _discoverSongs.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_MADE_FOR_YOU -> {
                    songRepository.getStarredSongs(true, 10).observeForever { _similarTracks.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_BEST_OF -> {
                    artistRepository.getStarredArtists(true, 20).observeForever { _bestOfArtists.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_RADIO_STATION -> {
                    artistRepository.getStarredArtists(true, 10).observeForever { _radioArtists.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_TOP_SONGS -> {
                    loadTopSongs()
                }
                Constants.HOME_SECTOR_STARRED_TRACKS -> {
                    songRepository.getStarredSongs(true, 20).observeForever { _starredTracks.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_STARRED_ALBUMS -> {
                    albumRepository.getStarredAlbums(true, 20).observeForever { _starredAlbums.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_STARRED_ARTISTS -> {
                    artistRepository.getStarredArtists(true, 20).observeForever { _starredArtists.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_NEW_RELEASES -> {
                    val currentYear = Calendar.getInstance().get(Calendar.YEAR)
                    albumRepository.getAlbums("byYear", 500, currentYear, currentYear).observeForever { list ->
                        _newReleases.value = list?.sortedByDescending { it.created }?.take(20) ?: emptyList()
                    }
                }
                Constants.HOME_SECTOR_FLASHBACK -> {
                    albumRepository.getDecades().observeForever { _flashbackYears.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_MOST_PLAYED -> {
                    albumRepository.getAlbums("frequent", 20, null, null).observeForever { _mostPlayedAlbums.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_LAST_PLAYED -> {
                    albumRepository.getAlbums("recent", 20, null, null).observeForever { _recentlyPlayedAlbums.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_RECENTLY_ADDED -> {
                    albumRepository.getAlbums("newest", 20, null, null).observeForever { _recentlyAddedAlbums.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_PINNED_PLAYLISTS -> {
                    val sortOrder = Preferences.getHomeSortPlaylists()
                    playlistRepository.getSortedPlaylistsPreview(sortOrder, 5).observeForever { _pinnedPlaylists.value = it ?: emptyList() }
                }
                Constants.HOME_SECTOR_SHARED -> {
                    if (Preferences.isSharingEnabled()) {
                        sharingRepository.getShares().observeForever { _shares.value = it ?: emptyList() }
                    }
                }
            }
        }
    ...
    fun changeTopSongsPeriod(period: Int) {
        ...
        val start = cal.timeInMillis
        val end = when (period) {
            0 -> { cal.set(Calendar.WEEK_OF_YEAR, currentWeek - 1); cal.timeInMillis }
            1 -> { cal.set(Calendar.WEEK_OF_YEAR, currentWeek - 4); cal.timeInMillis }
            2 -> { cal.set(Calendar.WEEK_OF_YEAR, currentWeek - 52); cal.timeInMillis }
            else -> start
        }
    ...
    private fun favoriteToStar(favorite: Favorite) {
        when {
            favorite.songId != null -> favoriteRepository.star(favorite.songId, null, null, object : StarCallback {
                override fun onSuccess() { favoriteRepository.delete(favorite) }
