...
class DynamicMediaSourceFactory(
    ...
    override fun createMediaSource(mediaItem: MediaItem): MediaSource {
        ...
        val useUpstream = streamingCacheSize.toInt() == 0
        ...
        return when {
            mediaItem.localConfiguration?.mimeType == MimeTypes.APPLICATION_M3U8 ||
                mediaItem.localConfiguration?.uri?.lastPathSegment?.endsWith(
                    ".m3u8",
                    ignoreCase = true
                ) == true -> {
                ...
                if (isTranscoding && OpenSubsonicExtensionsUtil.isTranscodeOffsetExtensionAvailable()) {
                    TranscodingMediaSource(mediaItem, progressiveFactory)
                } else {
