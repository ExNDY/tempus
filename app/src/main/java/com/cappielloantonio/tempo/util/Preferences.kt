...
object Preferences {
    private val settings: Settings by lazy {
        SharedPreferencesSettings(
            delegate = PreferenceManager.getDefaultSharedPreferences(
                App.getContext()
            )
        )
    }
    ...
    fun setNetworkPingTimeout(pingTimeout: String?) =
        settings.putString(NETWORK_PING_TIMEOUT, pingTimeout ?: "2")
    ...
    fun setOpenSubsonic(isOpenSubsonic: Boolean) =
        settings.putBoolean(OPEN_SUBSONIC, isOpenSubsonic)
    ...
    fun setAutoDownloadLyricsEnabled(isEnabled: Boolean) =
        settings.putBoolean(AUTO_DOWNLOAD_LYRICS, isEnabled)
    ...
    fun getInUseServerAddress(): String? =
        settings.getStringOrNull(IN_USE_SERVER_ADDRESS)?.takeIf { it.isNotBlank() } ?: getServer()
    ...
    fun setServerSwitchableTimer() =
        settings.putLong(NEXT_SERVER_SWITCH, System.currentTimeMillis())
    ...
    fun setBitrateVisible(bitrateVisible: Boolean) =
        settings.putBoolean(BITRATE_VISIBLE, bitrateVisible)
    ...
    fun setQuickActionVisible(quickActionVisible: Boolean) =
        settings.putBoolean(QUICK_ACTION_VISIBLE, quickActionVisible)
    ...
    fun setTrackNumberVisible(trackNumberVisible: Boolean) =
        settings.putBoolean(TRACK_NUMBER_VISIBLE, trackNumberVisible)
    ...
    fun setSkipSilenceMode(isSkipSilenceMode: Boolean) =
        settings.putBoolean(SKIP_SILENCE, isSkipSilenceMode)
    ...
    fun setShuffleModeEnabled(shuffleModeEnabled: Boolean) =
        settings.putBoolean(SHUFFLE_MODE, shuffleModeEnabled)
    ...
    fun getHideBottomNavbarOnPortrait(): Boolean =
        settings.getBoolean(HIDE_BOTTOM_NAVBAR_ON_PORTRAIT, false)
    ...
    fun getAudioTranscodeFormatWifi(): String =
        settings.getString(AUDIO_TRANSCODE_FORMAT_WIFI, "raw")

    @JvmStatic
    fun getAudioTranscodeFormatMobile(): String =
        settings.getString(AUDIO_TRANSCODE_FORMAT_MOBILE, "raw")
    ...
    fun setDataSavingMode(isDataSavingModeEnabled: Boolean) =
        settings.putBoolean(DATA_SAVING_MODE, isDataSavingModeEnabled)

    @JvmStatic
    fun isStarredArtistsSyncEnabled(): Boolean =
        settings.getBoolean(SYNC_STARRED_ARTISTS_FOR_OFFLINE_USE, false)

    @JvmStatic
    fun setStarredArtistsSyncEnabled(enabled: Boolean) =
        settings.putBoolean(SYNC_STARRED_ARTISTS_FOR_OFFLINE_USE, enabled)

    @JvmStatic
    fun isStarredAlbumsSyncEnabled(): Boolean =
        settings.getBoolean(SYNC_STARRED_ALBUMS_FOR_OFFLINE_USE, false)

    @JvmStatic
    fun setStarredAlbumsSyncEnabled(enabled: Boolean) =
        settings.putBoolean(SYNC_STARRED_ALBUMS_FOR_OFFLINE_USE, enabled)

    @JvmStatic
    fun isStarredSyncEnabled(): Boolean =
        settings.getBoolean(SYNC_STARRED_TRACKS_FOR_OFFLINE_USE, false)

    @JvmStatic
    fun setStarredSyncEnabled(enabled: Boolean) =
        settings.putBoolean(SYNC_STARRED_TRACKS_FOR_OFFLINE_USE, enabled)

    @JvmStatic
    fun showShuffleInsteadOfHeart(): Boolean =
        settings.getBoolean(MINI_SHUFFLE_BUTTON_VISIBILITY, false)

    @JvmStatic
    fun setShuffleInsteadOfHeart(enabled: Boolean) =
        settings.putBoolean(MINI_SHUFFLE_BUTTON_VISIBILITY, enabled)

    @JvmStatic
    fun getCustomCommandFirstButton(): String =
        settings.getString(CUSTOM_COMMAND_FIRST_BUTTON, "[heartID]")

    @JvmStatic
    fun getCustomCommandSecondButton(): String =
        settings.getString(CUSTOM_COMMAND_SECOND_BUTTON, "[repeatID]")

    @JvmStatic
    fun showServerUnreachableDialog(): Boolean =
        settings.getLong(SERVER_UNREACHABLE, 0) + 86400000 < System.currentTimeMillis()

    @JvmStatic
    fun setServerUnreachableDatetime() =
        settings.putLong(SERVER_UNREACHABLE, System.currentTimeMillis())
    ...
    fun isMusicDirectorySectionVisible(): Boolean =
        settings.getBoolean(MUSIC_DIRECTORY_SECTION_VISIBILITY, true)
    ...
    fun isReplayGainPreventClipping(): Boolean =
        settings.getBoolean(REPLAY_GAIN_PREVENT_CLIPPING, true)
    ...
    fun getStreamingCacheStoragePreference(): Int =
        settings.getString(STREAMING_CACHE_STORAGE, "0").toInt()

    @JvmStatic
    fun setStreamingCacheStoragePreference(preference: Int) =
        settings.putString(STREAMING_CACHE_STORAGE, preference.toString())

    @JvmStatic
    fun getDownloadStoragePreference(): Int = settings.getString(DOWNLOAD_STORAGE, "0").toInt()

    @JvmStatic
    fun setDownloadStoragePreference(preference: Int) =
        settings.putString(DOWNLOAD_STORAGE, preference.toString())
    ...
    fun setDownloadDirectoryUri(uri: String?) =
        settings.putString(DOWNLOAD_DIRECTORY_URI, uri ?: "")

    @JvmStatic
    fun getDefaultDownloadViewType(): String =
        settings.getString(DEFAULT_DOWNLOAD_VIEW_TYPE, Constants.DOWNLOAD_TYPE_TRACK)

    @JvmStatic
    fun setDefaultDownloadViewType(viewType: String) =
        settings.putString(DEFAULT_DOWNLOAD_VIEW_TYPE, viewType)
    ...
    fun isServerPrioritizedInTranscodedDownload(): Boolean =
        settings.getBoolean(AUDIO_TRANSCODE_DOWNLOAD_PRIORITY, false)
    ...
    fun getAudioTranscodeFormatTranscodedDownload(): String =
        settings.getString(AUDIO_TRANSCODE_FORMAT_DOWNLOAD, "raw")
    ...
    fun setHomeSectorList(extension: List<HomeSector>?) =
        settings.putString(HOME_SECTOR_LIST, gson.toJson(extension))

    @JvmStatic
    fun isLibraryMaterialCarouselEnabled(): Boolean =
        settings.getBoolean(HOME_LIBRARY_MATERIAL_CAROUSEL, true)

    @JvmStatic
    fun setLibraryMaterialCarouselEnabled(enabled: Boolean) =
        settings.putBoolean(HOME_LIBRARY_MATERIAL_CAROUSEL, enabled)
    ...
    fun showTempusUpdateDialog(): Boolean =
        settings.getLong(NEXT_UPDATE_CHECK, 0) + 86400000 < System.currentTimeMillis()
    ...
    fun isInstantMixUsable(): Boolean =
        settings.getLong(LAST_INSTANT_MIX, 0) + 10000 < System.currentTimeMillis()

    @JvmStatic
    fun setAllowPlaylistDuplicates(allowDuplicates: Boolean) =
        settings.putBoolean(ALLOW_PLAYLIST_DUPLICATES, allowDuplicates)
    ...
    fun getHomeSortPlaylists(): String =
        settings.getString(HOME_SORT_PLAYLISTS, DEFAULT_HOME_SORT_PLAYLISTS_SORT_ORDER)
    ...
    fun setSelectedEqualizer(selectedEqualizer: String) =
        settings.putString(SELECTED_EQUALIZER, selectedEqualizer)
    ...
    fun isSearchSortingChronologicallyEnabled(): Boolean =
        settings.getBoolean(SORT_SEARCH_CHRONOLOGICALLY, false)

    @JvmStatic
    fun getArtistDisplayBiography(): Boolean = settings.getBoolean(ARTIST_DISPLAY_BIOGRAPHY, true)

    @JvmStatic
    fun setArtistDisplayBiography(enabled: Boolean) =
        settings.putBoolean(ARTIST_DISPLAY_BIOGRAPHY, enabled)
    ...
    fun isAndroidAutoShuffleGenreSongsEnabled(): Boolean =
        settings.getBoolean(AA_SHUFFLE_GENRE_SONGS, false)

    @JvmStatic
    fun isAndroidAutoShuffleStarredTracksEnabled(): Boolean =
        settings.getBoolean(AA_SHUFFLE_STARRED_TRACKS, false)

    @JvmStatic
    fun isAndroidAutoShufflePlaylistsEnabled(): Boolean =
        settings.getBoolean(AA_SHUFFLE_PLAYLISTS, false)

    @JvmStatic
    fun setAndroidAutoShuffleGenreSongsEnabled(enabled: Boolean) =
        settings.putBoolean(AA_SHUFFLE_GENRE_SONGS, enabled)

    @JvmStatic
    fun getAndroidAutoStarredForMadeForYou(): Int =
        settings.getString(AA_STARRED_FOR_MADE_FOR_YOU, "0").toInt()
